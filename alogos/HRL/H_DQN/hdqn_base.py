...
import gym
import time
import alogos.HRL.H_DQN.core as core
...
class hdqn_base:
    '''
    定义了hdqn的base类，其实就是ddqn
    环境的动作空间一定是离散的
    '''
    def __init__(self, env_fn, input_dim, output_dim, policy=core.MLPpolicy, ac_kwargs=dict(), 
        ...
        # 建立Q_net, indim和outdim根据上层来决定，不要定死
        self.policy = policy(in_dim=self.input_dim, out_dim=self.output_dim, epsilon=epsilon, **ac_kwargs).to(self.device)
        ...
        # 初始buffer
        self.buffer = ReplayBuffer(self.input_dim, self.output_dim, replay_size, self.device)
    ...
    def get_action(self, o):
        ...
        '''
        a = self.policy.step(torch.as_tensor(o, dtype=torch.long).to(self.device))  # get动作的时候也要把o变成device形状！
        return a
