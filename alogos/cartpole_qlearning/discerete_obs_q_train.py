...
class Brain:
    ...
    def  __init__(self, num_states, num_actions):
        ...
        self.q_table = np.zeros(shape=(NUM_DIGITIZED**num_states, num_actions)) #uniform distributed sample with size
    
    def bins(self, clip_min, clip_max, num):
        #convert continous value to discrete value
        bins = np.linspace(clip_min, clip_max, num + 1)[1: -1]   #num of bins needs num+1 value
        return bins
    ...
    def digitize_state(self, observation):
        ...
        d_state = sum([x* (NUM_DIGITIZED**i) for i, x in enumerate(digitized)])
        return d_state
...
class Environment:
    ...
    def load_policy(self):
        self.agent.brain.q_table = np.load("q_table.npy.npy")
